...
$check_screen = mysqli_num_rows(mysqli_query($mysql, "SELECT userid FROM user WHERE screenname = '".$post_screenname."'"));
if($check_screen > 0){
	die("Screen name is already used, please insert another name.");
}

$pwdhash = md5($post_password);
if($pwdhash != $log_password){
	die("Current Password is invalid!");
}

$changequery = mysqli_query($mysql, "UPDATE user SET screenname = '".$post_screenname."', lastactive = NOW() WHERE userid = " . $log_userid);
if($changequery){
	die("success");
} else {
