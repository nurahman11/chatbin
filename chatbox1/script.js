...
function goRoomPassword(id, rm){
	...
	document.room_password.setAttribute("onSubmit","enterRoomPassword('"+id+"')");
	...
	ajax.onreadystatechange = function(){
		if(ajax.readyState == 4 && ajax.status == 200){
			...
			if(!isNaN(ajax_text) && parseInt(ajax_text) > 0){
				var locto = 'chatroom.php?id='+ajax_text;
				location.href = locto;
			} else {
...
function enterRoomPassword(id){
	...
	ajax.onreadystatechange = function(){
		if(ajax.readyState == 4 && ajax.status == 200){
			ajax_text = ajax.responseText;
			if(ajax_text === "true"){
				document.room_password.setAttribute("action","chatroom.php?id="+id);	
