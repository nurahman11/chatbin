...
$room = mysqli_query($mysql, "SELECT roomowner FROM room WHERE roomid = ".sql_req_esc($mysql, $c_id));
...
if(isset($_POST['tx'])){
	...
	if($c_msg != ""){
		mysqli_query($mysql, "INSERT INTO chat (roomid, userid, message) VALUES (".sql_req_esc($mysql, $c_id).", ".sql_req_esc($mysql, $log_userid).", '".sql_req_esc($mysql, $c_msg)."')");
	...
	if($log_userid == $room_owner && substr($c_msg,0,6) == "/kick "){
		...
		mysqli_query($mysql, "INSERT INTO chat (roomid, userid, message) VALUES (".sql_req_esc($mysql, $c_id).", 0, 'User \"".sql_req_esc($mysql, $kicked_user)."\" kicked.')");
	}
}
...
$o_sql .= "FROM chat c, user u WHERE c.userid = u.userid AND roomid = ".sql_req_esc($mysql, $c_id)." AND chatid > ".sql_req_esc($mysql, $o_last);
...
while($o_arr = mysqli_fetch_array($o_sql)){
	...
	if($o_arr["userid"] == $room_owner){
		...
		$addText .= html_esc($o_arr["screenname"]);
		$addText .= "</font>";
	...
	$chat_message = html_esc($o_arr["message"]);	
