...
<div id="floatdiv">
    <div id="roomlist">
        ...
        <div style="margin-right: 5px; margin-top: 5px; float: right;">
        	<form method="post" action="room.bc" enctype="application/x-www-form-urlencoded" name="searchform">
        		<input type="text" class="textbox" name="searchtxt" value="<?php echo html_esc($search); ?>" placeholder="Search Room" onFocus="timerPause();" onBlur="timerPause(false);" />
            	<button type="submit" class="button" name="searchbtn"><font class="symboltext">&#128269;</font></button>
            </form>
            <form method="post" action="room.bc" enctype="application/x-www-form-urlencoded" name="searchformhidden" style="display: none;">
                <input type="hidden" name="searchtxt" value="<?php echo html_esc($search); ?>" readonly />
            </form>
        </div>
        <div class="clear"></div>
        ...
        <div id="tablelist" class="autohidescroll">
        	<table width="100%" cellpadding="10" cellspacing="0" style="">
                <?php
					...
					$room_query		.= "FROM room";
					if($search != ""){
						$room_query	.= " WHERE roomname LIKE '%".sql_req_esc($mysql, $search)."%'";
					}
					...
					while($room_fetch = mysqli_fetch_array($room_sql)){
						$delusercount = "SELECT chatid FROM chat WHERE roomid = ".sql_req_esc($mysql, $room_fetch["roomid"])." AND time > DATE_SUB(NOW(), INTERVAL 30 MINUTE)";
						$delusercount = mysqli_num_rows(mysqli_query($mysql, $delusercount));
						if($delusercount == 0 && $room_fetch["roomid"] > 2){
							mysqli_query($mysql, "DELETE FROM chat WHERE roomid = ".sql_req_esc($mysql, $room_fetch["roomid"]));
							mysqli_query($mysql, "DELETE FROM room WHERE roomid = ".sql_req_esc($mysql, $room_fetch["roomid"]));
							continue;						
						}
						
						if($room_fetch['hidden'] == 1){
							continue;
						}
							
						$usercount = "SELECT userid FROM chat WHERE roomid = ".sql_req_esc($mysql, $room_fetch["roomid"])." AND time > DATE_SUB(NOW(), INTERVAL 5 MINUTE) AND userid <> 0 GROUP BY userid";
						$usercount = mysqli_num_rows(mysqli_query($mysql, $usercount));
									
						$roomnum++;
						if($room_fetch["roompassword"] == "" || $room_fetch["roomowner"] == $log_userid){
							$tr_onclick = "location.href = '".html_esc($room_fetch["roomid"])."-".roomname_esc($room_fetch["roomname"]).".bc'";
						} else {
							$tr_onclick = "goRoomPassword('".html_esc($room_fetch["roomid"])."', '".html_esc($room_fetch["roomname"])."')";
						}
				?>
                ...
                <tr onClick="<?php echo $tr_onclick; ?>" <?php if($roomnum%2 == 0){?> bgcolor="#A5D3A6"<?php } ?>>
                	...
                	<td align="left" width="520"><?php echo html_esc($room_fetch["roomname"]); ?></td>
                    <td align="right" width="150"><?php echo html_esc($usercount); ?> User</td>
                </tr>
        ...
        <div style="width: 500px; margin: auto; margin-top: 20px;">
            ...
            <table cellpadding="5" cellspacing="5" width="500" align="left">        
                <tr>
                    ...
                    <td align="left" width="300"><input type="text" class="textbox" name="screenname" style="width: 290px" maxlength="15" value="<?php echo html_esc($log_screen); ?>" /><br /><font style="font-size: 11px; color: #999999">4-15 character. AlphaNumeric only.</font></td>
                </tr>
                <tr>
