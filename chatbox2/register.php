...
if(isset($_POST["register"])){
	...
	if($post_email == "" || $post_password == "" || $post_password2 == "" || $post_screen == ""){
		...
		if(!preg_match($email_regex, $post_email)){
			...
			$check_email 	= mysqli_num_rows(mysqli_query($mysql, "SELECT userid FROM user WHERE email = '".sql_req_esc($mysql, $post_email)."'"));
			$check_screen 	= mysqli_num_rows(mysqli_query($mysql, "SELECT userid FROM user WHERE screenname = '".sql_req_esc($mysql, $post_screen)."'"));
			if($check_email > 0){
				...
				$do_register	= mysqli_query($mysql, "INSERT INTO user (email, password, screenname) VALUES('".sql_req_esc($mysql, $post_email)."','".sql_req_esc($mysql, $hash)."','".sql_req_esc($mysql, $post_screen)."')");
				if($do_register){
					$error_register		= "<font color=\"#28934E\">Register for &quot;".html_esc($post_email)."&quot; success.</font>";
				} else {
...
<div id="login">
    ...
    <form action="register.bc" method="post" enctype="application/x-www-form-urlencoded">
    <table cellpadding="5" cellspacing="5" width="400" align="left">        
        <tr>
            <th align="left" width="150" valign="top">E-Mail:</th>
            <td align="left" width="200"><input type="email" class="textbox" name="email" value="<?php echo html_esc($post_email); ?>" maxlength="50" /><br /><font style="font-size: 11px; color: #999999">Must valid E-Mail</font></td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td align="left"><input type="text" class="textbox" name="screenname" value="<?php echo html_esc($post_screen); ?>" maxlength="15" /><br /><font style="font-size: 11px; color: #999999">Used for chat name. 4-15 character. AlphaNumeric only.</font></td>
        </tr>
        <tr>
            <td align="center" colspan="2"><input type="submit" value="Register" class="button" name="register" /><br /><br /><a href="login.bc">Already have an account? Login here!</a></td>
        </tr>
    </table>
    </form>
    </div>
